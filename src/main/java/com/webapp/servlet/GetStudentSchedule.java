...
@WebServlet("/get-student-schedule")
public class GetStudentSchedule extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            sessions = em.createQuery("SELECT s FROM Session s WHERE s.student.idStudent = :studentID", Session.class)
                    .setParameter("studentID", studentID)
                    .getResultList();
        } catch (Exception e) {
            e.printStackTrace();
            response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            response.getWriter().write("{\"error\": \"Error fetching schedule\"}");
            return;
        }
